...
"""
Features Extraction Signals WESAD dataset

"""
import sys
sys.path.append("/home/black/Github_AIGOT/Stress-Detection/ProcessUtils") 

from Noise_reductions import *
...
def save_spectrogram(RR_list, output_path, subject_id, label, num_partial_lists=1):
    np.seterr(divide='ignore')     
    """Save spectrograms from a list of RR intervals as 256x256 images."""
    int_to_label = {0: 'non-stress', 1: 'stress'}

    length_rr = len(RR_list)
    sub_list_length = length_rr // num_partial_lists  # Calculate the length of each partial list
    path_subjects = []

    for i in range(num_partial_lists):
        start_idx = i * sub_list_length
        end_idx = (i + 1) * sub_list_length
        sub_rr = RR_list[start_idx:end_idx]
        # plt.figure(figsize=(8, 8))  # Set the figure size to 256x256 pixels
        rr_seconds = np.array(sub_rr) / 1000.0

        if len(sub_rr) <= 1:
            # Skip empty or very short RR intervals
            continue

        try:
            plt.specgram(x=rr_seconds, Fs=64, NFFT=30, noverlap=28, cmap="twilight")
        except ValueError as e:
            print(f"Error: {e}")
            continue

        plt.axis('off')  # Turn off axis
        plt.savefig(f"{output_path}/{label}_S{subject_id}_{i}.png", bbox_inches='tight', pad_inches=0, transparent=True)
        plt.close()

        path_subjects.append(f"{output_path}/{label}_S{subject_id}_{i}.png")

    return path_subjects
    
def get_rr_interval_spectrogram(ppg_seg, window_length, label, path_spectral, subject_id,ensemble, ma_usage):
    int_to_label = {0: 'non-stress', 1: 'stress'}
    ...
    RR_list, _, _ = calc_RRI(peak, fs)
    # HR = calc_heartrate(RR_list)
    
    if len(RR_list) <= 3:
        return []
    
    label_str = int_to_label[label]
    RR_path_spectral = os.path.join(path_spectral, label_str)
    
    if not os.path.exists(RR_path_spectral):
        os.makedirs(RR_path_spectral)
    
    path_subjectID = save_spectrogram(RR_list, RR_path_spectral,subject_id, label)
    
    RR_interval = {'RR_interval': path_subjectID, 'label': label}
