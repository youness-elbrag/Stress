...
def combine_files(subjects):
    df_list = []
    for s in subjects:
        df = pd.read_csv(f'{save_path}{subject_feature_path}/S{s}_feats_4.csv', index_col=0)
        df['subject'] = s
        df_list.append(df)

    df = pd.concat(df_list)
    df['label'] = (df['0'].astype(str) + df['1'].astype(str)).apply(lambda x: 1 if x.startswith('True') else 0)
    df.drop(['0', '1'], axis=1, inplace=True)

    df.reset_index(drop=True, inplace=True)

    df.to_csv(save_path + merged_path)
...
def make_patient_data(subject_id, ma_usage, main_path):
    global save_path
    global WINDOW_IN_SECONDS
...
def create_arg_parser():
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="featureTestTraining/binary/",
        help="Path to save the features (default: featureTestTraining/binary/)",
    )
...
if __name__ == "__main__":
    ...
    feat_names = None
    # save_path = 'featureTest/binar/ens/3'
    ...
    for n in NOISE:
        ...
        if not os.path.exists(save_path + subject_feature_path):
            os.makedirs(save_path + subject_feature_path)
        
            
        for patient in subject_ids:
            print(f'Processing data for S{patient}...')
            window_len = make_patient_data(patient, BP, main_path)
